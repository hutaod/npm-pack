...
// 获取 pack 内容
async function getPackFiles(packPath) {
  try {
    // 1. 获取基础文件
    const fileList = (await readdir(packPath)).filter(filterFilePath).map(item => `${packPath}/${item}`)

    // 2. 获取node_modules内容
    const moduleList = await getModuleList({ path: packPath, bundleDependenciesKey: "dependencies" });
...
async function getModuleList(opt) {
  const moduleList = (
    await bundled({ path: opt.path, bundleDependenciesKey: opt.bundleDependenciesKey })
  )
  for (let index = 0; index < moduleList.length; index++) {
    moduleList[index] = await getRealPath(moduleList[index])
  }
  return moduleList;
}
